...
float lfo_phase_increment = LFO_FREQUENCY * LFO_TABLE_SIZE / SAMPLE_RATE;
...
float get_lfo_value(float*, float);
...
void fill_audio_buffer(uint32_t *buf, int16_t *table, uint8_t is_half)
{
	...
	// calculate phase increment for frequency modulation
	float modulated_frequency = FREQUENCY + (get_lfo_value(lfo_table, lfo_phase_increment) * 25.0f);
	static float base_phase_increment = FREQUENCY * FREQUENCY_CORRECTION * TABLE_SIZE / (2*SAMPLE_RATE);
	float phase_increment = base_phase_increment + (modulated_frequency * TABLE_SIZE / (2*SAMPLE_RATE));
...
float get_lfo_value(float *table, float phase_increment)
{
